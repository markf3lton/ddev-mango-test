...
(function enableDisablePlugin($,window,Drupal,once){Drupal.behaviors.projectBrowserPluginSourceDrag={attach(context){if(!Drupal.tableDrag||!Drupal.tableDrag.project_browser)return;function updateLastPlaced(table,rowObject){const $rowObject=$(rowObject);if(!$rowObject.is('.drag-previous')){table.find('.drag-previous').removeClass('drag-previous');$rowObject.addClass('drag-previous');}}function updateSourcePluginWeights(table,region){let weight=-Math.round(table.find('.draggable').length/2);table.find(`.status-title-${region}`).nextUntil('.status-title').find('select.source-weight').val(()=>{weight+=1;return weight;});}const table=$('#project_browser');const tableDrag=Drupal.tableDrag.project_browser;tableDrag.row.prototype.onSwap=function swapHandler(){updateLastPlaced(table,this);};tableDrag.onDrop=function dropHandler(){const dragObject=this;const $rowElement=$(dragObject.rowObject.element);const regionRow=$rowElement.prevAll('tr.status-title').get(0);const regionName=regionRow.classList[1].replace('status-title-','');const regionField=$rowElement.find('select.source-status-select');if(!regionField.is(`.source-status-${regionName}`)){const weightField=$rowElement.find('select.source-weight');const oldRegionName=weightField[0].className.replace(/([^ ]+[ ]+)*source-weight-([^ ]+)([ ]+[^ ]+)*/,'$2');regionField.removeClass(`source-status-${oldRegionName}`).addClass(`source-status-${regionName}`);weightField.removeClass(`source-weight-${oldRegionName}`).addClass(`source-weight-${regionName}`);regionField.val(regionName);}updateSourcePluginWeights(table,regionName);};$(once('source-status-select','select.source-status-select',context)).on('change',function addBehaviorOnce(){const row=$(this).closest('tr');const select=$(this);tableDrag.rowObject=new tableDrag.row(row[0]);const regionMessage=table.find(`.status-title-${select[0].value}`);const regionItems=regionMessage.nextUntil('.status-title');if(regionItems.length)regionItems.last().after(row);else regionMessage.after(row);updateSourcePluginWeights(table,select[0].value);updateLastPlaced(table,row);if(!tableDrag.changed){$(Drupal.theme('tableDragChangedWarning')).insertBefore(tableDrag.table).hide().fadeIn('slow');tableDrag.changed=true;}select.trigger('blur');});}};})(jQuery,window,Drupal,once);;
